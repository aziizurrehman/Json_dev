...
const ReqParamModel = require("../model/reqAccountStatement");
  ...
  async perform() {
    try {
        ...
        return new APIResponse(
          ...
          new APIError(
           ...
           "400",
            ...
            bodyValidationResponse.error.message.includes('date format')?"Date Format Must Be YYYY-MM-DD":bodyValidationResponse.error.message, //,
            bodyValidationResponse.error.name,
            bodyValidationResponse.error.stack
          )
        );
      }
      ...
      const clientService = new ClientService(
        config.get("api.v1.accountStatement.api_type"),
        this.commonHeaders
      );
      ...
      if (response.data.ResultCode == '00' || response.data.ResultCode == '21') {
